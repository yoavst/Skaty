...
object Help {
    ...
    fun generate(protocol: IProtocolMarker<*>): String {
        ...
        val info = properties.map { Triple(it.name, it.returnType.format(), it.getter.call(defaultValue)) }
        ...
        return info.joinToString(separator = "\n") { (name, type, value) ->
            "${name.padEnd(maxNameLen).colorize(ParameterNameColor)} : ${type.padEnd(maxTypeLen).colorize(FieldColor)} = (${value.toString().colorize(DefaultValueColor)})"
        }
